 ...
 *
 */
package edu.dnatools.basepairs;
...
END"""
    ...
    // previous residue O3'
    public static final String STANDARD_PHOSPHATE_FULL = """
            SEQRES   1 A    1  A
            ...
            END""";
    public static final String STANDARD_PHOSPHATE = """
            SEQRES   1 A    1  A
            ATOM      1  P     A A   1       0.000   0.000   0.000
            ATOM      2  OP1   A A   1      -1.208   0.854  -0.000
            ATOM      3  OP2   A A   1       1.208   0.854   0.000
            ATOM      4  O5'   A A   1       0.000  -0.930   1.315
            END""";
    // previous residue O3'
    public static final String STANDARD_PHOSPHATE_O3 = """
            SEQRES   1 A    1  A
            ATOM      1  O3'   A A   1       0.000  -0.929  -1.315
            END""";
   ...
   //        DNAspecific = Arrays.asList("DC", "C", "CYT");
    ...
    protected double[][] phosphateParameters;
    ...
    protected List<Matrix4d> phosphateReferenceFrames = new ArrayList<>();
    ...
    public BasePairParameters analyze() {
        ...
        this.phosphateParameters = new double[2*pairs.size()][6];
        Matrix4d base;
        Matrix4d lastStep;
        ...
        for (int i = 0; i < pairs.size(); i++) {
            ...
            base = this.baseReferenceFrame(pairs.get(i).getFirst());
            System.arraycopy(pairParameters, 0, pairingParameters[i], 0, 6);
            if (i != 0) {
                phosphate = this.phosphateReferenceFrame(pairs.get(i - 1).getFirst(), pairs.get(i).getFirst());
                lastStep.invert();
            ...
            if (base != null && phosphate != null) {
                phosphateReferenceFrames.add((Matrix4d) phosphate.clone());
                Matrix4d b = (Matrix4d) base.clone();
                b.invert();
                b.mul(phosphate);
                double[] pparms = calculateTp(b);
                System.arraycopy(pparms, 0, phosphateParameters[2*i], 0, 6);
            } else {
                phosphateReferenceFrames.add(null);
                for (int j = 0; j < 6; j++) phosphateParameters[2*i][j] = 0;
            }

            base = this.baseReferenceFrame(pairs.get(i).getSecond());
            if (i != pairs.size() - 1) {
                phosphate = this.phosphateReferenceFrame(pairs.get(i + 1).getSecond(), pairs.get(i).getSecond());
            } else {
                phosphate = null;
            }
            if (phosphate != null && base != null) {
                phosphateReferenceFrames.add((Matrix4d) phosphate.clone());
                Matrix4d b = (Matrix4d) base.clone();
                b.invert();
                b.mul(phosphate);
                double[] pparms = calculateTp(b);
                System.arraycopy(pparms, 0, phosphateParameters[2*i + 1], 0, 6);
            } else {
                phosphateReferenceFrames.add(null);
                for (int j = 0; j < 6; j++) phosphateParameters[2*i+1][j] = 0;
            }
        }
        return this;
    }
...
/*
Returns 4x4 matrix reference frame for a single base
 */
    public Matrix4d baseReferenceFrame(Group g) {
        Integer type1 = BASE_MAP.get(g.getPDBName());
        if (type1 == null) return null;
        SuperPosition sp = new SuperPositionSVD(true);
        Structure s;
        PDBFileReader pdbFileReader = new PDBFileReader();
        try {
            s = pdbFileReader.getStructure(new ByteArrayInputStream(STANDARD_BASES[type1].getBytes()));
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
        Group std1 = s.getChain("A").getAtomGroup(0);
        int N = std1.getAtoms().size();
        Point3d[] pointref = new Point3d[std1.getAtoms().size()];
        Point3d[] pointact = new Point3d[std1.getAtoms().size()];
        int count = 0;
        for (Atom a : std1.getAtoms()) {
            if (g.getAtom(a.getName()) == null) return null;
            pointref[count] = a.getCoordsAsPoint3d();
            pointact[count] = g.getAtom(a.getName()).getCoordsAsPoint3d();
            count++;
        }
        assert count == std1.getAtoms().size();
        return (Matrix4d)sp.superposeAndTransform(pointact, pointref).clone();
    }



    public Matrix4d phosphateReferenceFrame(Group prev, Group current) {
        SuperPosition sp = new SuperPositionSVD(true);
        Structure sO3, s;
        PDBFileReader pdbFileReader = new PDBFileReader();
        try {
            sO3 = pdbFileReader.getStructure(new ByteArrayInputStream(STANDARD_PHOSPHATE_O3.getBytes()));
            s = pdbFileReader.getStructure(new ByteArrayInputStream(STANDARD_PHOSPHATE.getBytes()));
        } catch (IOException e) {
            log.error(e.getMessage());
            e.printStackTrace();
            return null;
        }
        Group O3 = sO3.getChain("A").getAtomGroup(0);
        Group rest = s.getChain("A").getAtomGroup(0);

        Point3d[] pointref = new Point3d[O3.getAtoms().size()+rest.getAtoms().size()];
        Point3d[] pointact = new Point3d[O3.getAtoms().size()+rest.getAtoms().size()];

        int count = 0;

        for (Atom a : O3.getAtoms()) {
            if (prev.getAtom(a.getName()) == null) {
                log.info("Missing atom " + a.getName());
                return null;
            }
            pointref[count] = a.getCoordsAsPoint3d();
            pointact[count] = prev.getAtom(a.getName()).getCoordsAsPoint3d();
            count++;
        }

        for (Atom a: rest.getAtoms()) {
            if (current.getAtom(a.getName()) == null) {
                log.info("Missing atom " + a.getName());
                return null;
            }
            pointref[count] = a.getCoordsAsPoint3d();
            pointact[count] = current.getAtom(a.getName()).getCoordsAsPoint3d();
            count++;
        }

        assert count == O3.getAtoms().size()+rest.getAtoms().size();
        return (Matrix4d)sp.superposeAndTransform(pointact, pointref).clone();
    }

    public double[] get30Coordinates(int pos) {
        double[] result = new double[30];
        for (int i = 0; i < 6; i++) {
            result[i] = pairingParameters[pos-1][i];
            result[6+i] = phosphateParameters[2*pos][i];
            result[12+i] = stepParameters[pos][i];
            result[18+i] = phosphateParameters[2*pos-1][i];
            result[24+i] = pairingParameters[pos][i];
        }
        return result;
    }
    ...
    public double[] reversePacking(double[] data) {
        ...
        for (int i = 0; i < 6; i++) {
            result[i] = data[24+i];
            result[6+i] = data[18+i];
            result[12+i] = data[12+i];
            result[18+i] = data[6+i];
            result[24+i] = data[i];
            if (i == 0 || i == 3) {
                result[i] = -result[i];
                result[12+i] = -result[12+i];
                result[24+i] = -result[24+i];
            }
        }
        return result;
    }
     ...
     */
    public static boolean match(char a, char b, boolean RNA) {
    ...
    public double[][] getPhosphateParameters() {
        return phosphateParameters;
    }
